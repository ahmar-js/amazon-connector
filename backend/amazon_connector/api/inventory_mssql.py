...
def save_inventory_report_to_mssql(csv_path: str, latest_report: dict, marketplace_code: str, items_count: int) -> dict:
    ...
    report_type = latest_report.get('reportType')
    period = latest_report.get('period')
    created_time = latest_report.get('createdTime')
    data_start = latest_report.get('dataStartTime')
